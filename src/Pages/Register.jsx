...
export default function RegisterWithOtp() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const res = await axios.post(API.AUTH_REGISTER_OTP, {
        username: form.username.trim(),
        email: form.email.trim(),
        password: form.password,
        name: form.name.trim(),
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      // Verifikasi OTP
      const res = await axios.post(API.AUTH_VERIFY_OTP, {
        username: form.username.trim(),
        email: form.email.trim(),
        otp: form.otp.trim(),
      });
      ...
      // Login otomatis
      const loginRes = await axios.post(API.AUTH_LOGIN, {
        usernameOrEmail: form.username.trim(),
        password: form.password,
      });
  ...
  const handleResendOtp = async () => {
    ...
    setSuccess("");
    try {
      const res = await axios.post(API.AUTH_RESEND_OTP, {
        username: form.username.trim(),
        email: form.email.trim(),
      });
      setSuccess(res.data.message || "OTP baru sudah dikirim ke email Anda");
